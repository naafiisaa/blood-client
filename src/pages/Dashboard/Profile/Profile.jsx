...
const Profile = () => {
  ...
  const onSubmit = (data) => {
    ...
    if (!hasChanged) return Swal.fire({ icon: 'info', title: 'No Changes', text: 'No changes were made to the profile.' });
    ...
    axiosSecure.patch(`/users/${userInfo._id}`, data)
      ...
      .catch(() => Swal.fire({ icon: 'error', title: 'Update Failed', text: 'Error updating profile.' }));
  };
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-3xl font-semibold" style={{ color: colors.primary }}>My Profile</h2>
        {!isEditable ? (
          <motion.button
            type="button"
            onClick={() => setIsEditable(true)}
            ...
            style={{ backgroundColor: colors.secondary, color: colors.background, borderColor: colors.accent }}
            title="Edit Profile"
          >
            <FiEdit3 /> Edit
          </motion.button>
          ...
          <motion.button
            ...
            style={{ backgroundColor: colors.primary, color: colors.background, borderColor: colors.accent }}
            title="Save Changes"
          >
            <FiSave /> Save
          </motion.button>
      ...
      <p className="mb-6 flex items-center gap-2" style={{ color: colors.text }}>
        <FiInfo style={{ color: colors.primary }} /> Manage your personal information
      </p>
      ...
      {userInfo && (
        <form id="profile-form" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {['name', 'avatar', 'bloodGroup', 'district', 'upazila'].map(field => (
              <div key={field}>
                <label className="label-text" style={{ color: colors.text }}>{field.charAt(0).toUpperCase() + field.slice(1)}</label>
                {field === "bloodGroup" ? (
                  <select
                ...
                {errors[field] && <span className='text-red-600 text-sm'>{field} is required</span>}
              </div>
            ))}

            <div>
              <label className="label-text" style={{ color: colors.text }}>Email</label>
              <input
                type="email"
                {...register("email")}
                className="input input-bordered w-full"
